(function() {
  ...
  var Crawler = {
    ...
    /*
     * This function lazily loads in the packages
     */
    getPackages: function(callback) {
      if (!this.packages) {
        fs.readFile(this.cache, function(err, data) {
          if (err) {
            colog.error("Could not read " + this.cache);
            return;
          }
    ...
    find: function(regex, callback) {
      this.getPackages(function(packages) {
        var results = [];
    ...
    /*
     * This funcion finds all dependent packages of a given one
     */
    grabDependencies: function(lib) {
      ...
      function findDep(i) {
        ...
        that.getPackages( function(packages) {
          console.log('test');
          var maybePile = [];
          var found;
