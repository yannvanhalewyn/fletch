(function() {
  ...
  var link = {
    ...
    toFile: function(pkgName, version, filename) {
      return "https:" + this._host(pkgName, version, filename);
    },

    apiSearch: function(name) {
      var url = "https://api.cdnjs.com/libraries?search={{name}}" +
                "&fields=version,description,assets,dependencies";
      return url.replace("{{name}}", name);
    },

    apiAssets: function(name) {
      var url = "https://api.cdnjs.com/libraries/{{name}}" +
                "?fields=assets";
      return url.replace("{{name}}", name);
    }
